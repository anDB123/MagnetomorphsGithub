...
def makeRegionOfUniformityPlot(ax):
    centre = 0, 0
    z1 = [
    ...
    y1, x1 = np.meshgrid(np.linspace(-8, 8, 17),
                         np.linspace(-4, 4, 9))

    y2, x2 = np.meshgrid(np.linspace(-8, 8, 17),
                         np.linspace(-4, 4, 9))
    ...
    rect = patches.Rectangle((-2, -3), 4, 6, linewidth=1, edgecolor='r', facecolor='none')
    ax.add_patch(rect)

    ax.text(0, 0, "Uniform Region\n\n(4cm by 6cm)", ha="center", va="center", color='r', fontsize=13)

    ax.axvline(x=0, color='k', dashes=[2, 2])
    ax.axhline(y=0, color='k', dashes=[2, 2])
