...
for i in range(0, 8):
    ...
    initial_difference_image = DifferenceImageObject(image_array[0], bg_image, crop_29, useOriginalImageForFilter,
                                                     any_colour_reduce_noise, any_colour_noise_reduction_array)
    initialPolymerSample = PolymerSample(thickness, currents_array[0])
    physicalModel = PhysicalModelManyCurvesOptimised(currents_array[0], temp_properties_array, [0] * number_of_curves)
...
physicalModel = PhysicalModelManyCurvesOptimised(temp_current, temp_properties_array, [0] * number_of_curves)
